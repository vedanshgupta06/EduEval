...
EduEval AI uses OCR.Space for image and PDF answer sheets.
...
EDUEVAL_OCR_SPACE_ENGINE=2
EDUEVAL_OCR_SPACE_MAX_UPLOAD_BYTES=900000
EDUEVAL_OCR_SPACE_IMAGE_MAX_SIDE=1600
```

Engine 2 is the default because it works reliably with OCR.Space's free API
endpoint. Set `EDUEVAL_OCR_SPACE_ENGINE=3` only if your OCR.Space key supports
it. The checked-in local `.env` may use OCR.Space's demo key `helloworld`, but
that is rate limited. Use your own key for real testing.
